...
function ManipulationAuthority:loadConfig()
	...
	if mafSandbox then
		...
		self.config.ISDestroyStuffActionProtection = mafSandbox.Get("ISDestroyStuffActionProtection", true)
		self.config.ISDismantleActionProtection = mafSandbox.Get("ISDismantleActionProtection", true)
		self.config.ISMoveablesActionProtection = mafSandbox.Get("ISMoveablesActionProtection", true)
		self.config.ISMoveableSpritePropsProtection = mafSandbox.Get("ISMoveableSpritePropsProtection", true)
	end
...
function ManipulationAuthority:processAction(phase, context)
	...
	if not success then
		logger:error("Error executing listeners for phase: " .. eventName, { error = tostring(err) })
		return false
	end
